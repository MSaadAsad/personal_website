...
export default function ProvinceComparison(){
  ...
  return <main className="compare-shell"><header><a href={`/pakistan-map#map=${new URLSearchParams(location.hash.slice(1)).get('map')||''}`}>← Back to map</a><span>NAYA NAQSHA · COMPARISON</span></header><section className="compare-head"><div><span>PROVINCE PLAN · {config.l.toUpperCase()}</span><h1>{config.n}</h1><p>{rows.length} populated provinces and territories compared using the same matched source data as the profiles.</p></div><div><b>{rows.length}</b><span>MAP UNITS</span></div></section><nav className="metric-tabs" aria-label="Choose comparison metric">{METRICS.map(m=><button key={m.key} className={metric===m.key?'selected':''} onClick={()=>setMetric(m.key)}>{m.label}</button>)}</nav><section className="comparison-view"><div className="comparison-title"><div><span>{meta.kind==='share'?'COMPOSITION':meta.kind==='rate'?'PERCENTAGE COMPARISON · 0–100%':'RANKED COMPARISON'}</span><h2>{meta.label}</h2></div><p>{meta.unit}{meta.lowBetter?' · lower is better':''}</p></div>{metric==='population'?<div className="population-composition"><div className="pie" style={{background:`conic-gradient(${pie})`}} role="img" aria-label="Population share by map unit"><span><b>{(totalPopulation/1e6).toFixed(1)}m</b>mapped population</span></div><ol>{ranked.map((r,i)=><li key={r.id}><i style={{background:r.color}}/><span>{String(i+1).padStart(2,'0')}</span><b>{r.name}<em>{r.kind}</em></b><strong>{format(metric,r.population)}</strong><small>{totalPopulation?(r.population/totalPopulation*100).toFixed(1):'0.0'}%</small></li>)}</ol></div>:<div className="comparison-column-chart"><div className="column-axis" aria-hidden="true">{[1,.75,.5,.25,0].map(stop=><b key={stop}>{formatAxis(metric,maximum*stop)}</b>)}</div><ol className="comparison-columns" style={{gridTemplateColumns:`repeat(${ranked.length},minmax(92px,1fr))`}}>{ranked.map((r,i)=><li key={r.id}><strong>{format(metric,Number(r[metric]))}</strong><span className="column-track"><i style={{height:`${Number(r[metric])/maximum*100}%`}}/></span><div className="column-label"><span>{String(i+1).padStart(2,'0')}</span><b>{r.name}<em>{r.kind}</em></b></div></li>)}</ol></div>}</section><footer><span>Statistics include provinces and territories. National Assembly and Senate calculations include provinces only.</span><button onClick={()=>navigator.clipboard.writeText(location.href)}>Copy comparison link</button></footer></main>;
}
