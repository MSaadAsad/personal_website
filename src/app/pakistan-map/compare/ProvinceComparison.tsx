...
const districtKeys=(feature:Feature,includeKeamari:boolean)=>{const raw=normalise(feature.properties.district_name);return [raw,...(includeKeamari&&raw==='westkarachi'?['keamari']:[])].map(key=>aliases[key]||key)};
...
export default function ProvinceComparison(){
  ...
  const rows=useMemo<Row[]>(()=>{
    ...
    return config.p.map(([id,name,color,kind],owner)=>{
      ...
      const bases={...totals};
      members.forEach(f=>{const tehsil=config.l==='tehsils'?tehsils.get(String(f.properties.tehsil_code)):null;districtKeys(f,config.l==='districts').forEach(districtKey=>{const district=data.districts[districtKey];if(config.l==='districts'&&!district){population+=regionalDistrictPopulation2017(districtKey)||0;return}const pop=config.l==='tehsils'?(tehsil?.p||0):(district?.p||regionalDistrictPopulation2017(districtKey)||0);population+=pop;if(!district)return;if(district.l!=null&&district.i!=null&&pop){literate+=district.l/(district.l+district.i)*pop;literacyBase+=pop}if(district.u!=null&&district.p&&pop){urban+=district.u/district.p*pop;urbanBase+=pop}if(config.l==='districts'&&district.oos!=null){outOfSchool+=district.oos;oosMatches++}const source:{[K in keyof typeof totals]:number|null}={matric:district.mat,consumption:district.cons,lfpr:district.lfpr,food:district.fi,internet:district.net,electricity:district.elec,mpi:district.mpi};(Object.keys(totals) as (keyof typeof totals)[]).forEach(k=>{if(source[k]!=null&&pop){totals[k]+=Number(source[k])*pop;bases[k]+=pop}})})});
      const memberDistricts=new Set(members.flatMap(f=>districtKeys(f,config.l==='districts')));
      const census=config.l==='districts'?aggregateCensus(memberDistricts,censusDetail):null;
