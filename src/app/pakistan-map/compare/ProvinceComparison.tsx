...
export default function ProvinceComparison(){
  ...
  const ranked=[...rows].filter(r=>r[metric]!=null&&(!perCapita||r.population>0)).sort((a,b)=>metricValue(b)-metricValue(a));
  const maximum=meta.kind==='rate'?100:Math.max(...ranked.map(metricValue),1);
  ...
  const kicker=composition?'COMPOSITION':perCapita?'PER-CAPITA COMPARISON':meta.kind==='rate'?'PERCENTAGE COMPARISON · 0–100%':'RANKED COMPARISON';
  ...
  return <main className="compare-shell"><header><a href={`/pakistan-map#map=${new URLSearchParams(location.hash.slice(1)).get('map')||''}`}>← Back to map</a><span>NAYA NAQSHA · COMPARISON</span></header><section className="compare-head"><div><span>PROVINCE PLAN · {config.l.toUpperCase()}</span><h1>{config.n}</h1><p>{rows.length} populated provinces and territories compared using the same matched source data as the profiles.</p></div><div><b>{rows.length}</b><span>MAP UNITS</span></div></section><nav className="metric-tabs" aria-label="Choose comparison metric">{METRICS.map(m=><button key={m.key} className={metric===m.key?'selected':''} onClick={()=>setMetric(m.key)}>{m.label}</button>)}</nav><section className="comparison-view"><div className="comparison-title"><div><span>{kicker}</span><h2>{meta.label}</h2></div><p>{unitLabel}{meta.lowBetter?' · lower is better':''}</p></div>{metric==='outOfSchool'&&<div className="comparison-mode" role="group" aria-label="Out-of-school comparison basis"><button className={outOfSchoolMode==='total'?'selected':''} onClick={()=>setOutOfSchoolMode('total')}>Total children</button><button className={outOfSchoolMode==='perCapita'?'selected':''} onClick={()=>setOutOfSchoolMode('perCapita')}>Per 1,000 residents</button></div>}{composition?<div className="population-composition"><div className="pie" style={{background:`conic-gradient(${pie})`}} role="img" aria-label={`${meta.label} share by map unit`}><span><b>{compositionCentre}</b>{compositionLabel}</span></div><ol>{ranked.map((r,i)=><li key={r.id}><i style={{background:r.color}}/><span>{String(i+1).padStart(2,'0')}</span><b>{r.name}<em>{r.kind}</em></b><strong>{formatValue(r)}</strong><small>{compositionTotal?(metricValue(r)/compositionTotal*100).toFixed(1):'0.0'}%</small></li>)}</ol></div>:<div className="comparison-column-chart"><div className="column-axis" aria-hidden="true">{[1,.75,.5,.25,0].map(stop=><b key={stop}>{formatScale(maximum*stop)}</b>)}</div><ol className="comparison-columns" style={{gridTemplateColumns:`repeat(${ranked.length},minmax(92px,1fr))`}}>{ranked.map((r,i)=><li key={r.id}><strong>{formatValue(r)}</strong><span className="column-track"><i style={{height:`${metricValue(r)/maximum*100}%`}}/></span><div className="column-label"><span>{String(i+1).padStart(2,'0')}</span><b>{r.name}<em>{r.kind}</em></b></div></li>)}</ol></div>}{perCapita&&<p className="comparison-note">Calculated from out-of-school children divided by total 2023 population. This is per 1,000 residents, not per 1,000 children aged 5–16.</p>}</section><footer><span>Statistics include provinces and territories. National Assembly and Senate calculations include provinces only.</span><button onClick={()=>navigator.clipboard.writeText(location.href)}>Copy comparison link</button></footer></main>;
}
