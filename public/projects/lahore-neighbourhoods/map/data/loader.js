window.BASELINE_READY=(async()=>{
  const response=await fetch('data/mobility.json.gz?v=31',{cache:'force-cache'});
  ...
  const placeResponse=await fetch('data/territory-places.json?v=1',{cache:'force-cache'});
